...
test('render placeholder', () => {
  ...
  render(<EmailInput name={name} value={value} onChange={mockChange} placeholder={placeholder} />);
  expect(screen.getByPlaceholderText(placeholder)).toBeInTheDocument();
});

test('render label', () => {
	const label = 'label';
  render(<EmailInput name={name} value={value} onChange={mockChange} label={label} />);
  expect(screen.getByText(label)).toBeInTheDocument();
});

test('render readOnly', () => {
  render(<EmailInput name={name} value={value} onChange={mockChange} readOnly />);
  expect(screen.getByDisplayValue(value)).toHaveAttribute('readOnly');
});

test('render required', () => {
  render(<EmailInput name={name} value={value} onChange={mockChange} required />);
  expect(screen.getByDisplayValue(value)).toHaveAttribute('required');
});

test('render disabled', () => {
  render(<EmailInput name={name} value={value} onChange={mockChange} disabled />);
