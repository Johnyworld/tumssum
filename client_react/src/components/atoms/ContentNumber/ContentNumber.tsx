...
const ContentNumber: React.FC<ContentNumberProps> = ({
  ...
  const handleBlur: React.FocusEventHandler<HTMLDivElement> = useCallback(
    (e) => {
      const removeCharacters = numberUtil.removeCharacters(
  ...
  useEffect(() => {
    if (ref.current)
      setInnerText(numberUtil.removeCharacters(String(value || '')));
  }, []);
  ...
  return isNatural ? (
    ...
    <div className='content-number__container'>
      {component}
      <div
        className='content-number__svg pos-center-y pointer never-drag'
        onClick={() => setNegative(!isNegative)}
      >
        {isNegative ? (
          <svg
            width='25'
